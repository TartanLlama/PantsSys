  ...
  PantsUI()
    ...
    if (!m_window) {
      ...
      throw std::runtime_error{
          std::string{"Unable to create window: "}.append(SDL_GetError())};
    }

    m_surface = SDL_GetWindowSurface(m_window);
    if (!m_surface) {
      throw std::runtime_error{
          std::string{"Unable to get window surface: "}.append(SDL_GetError())};
    }
    ...
    if (!m_renderer) {
      ...
      if (!m_renderer) {
        throw std::runtime_error{
            std::string{"Unable to get renderer: "}.append(SDL_GetError())};
      }
    }
    ...
    if (!(IMG_Init(img_flags) & img_flags)) {
      throw std::runtime_error{
          std::string{"Unable to init SDL_image: "}.append(IMG_GetError())};
    }
