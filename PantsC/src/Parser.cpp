...
namespace pants {
    ...
    Maybe<ASTNodeUP> Parser::ParseOperand() {
        ...
        IssueDiagnostic(operand.value(), "Unexpected token {}", operand.value().ToString());
    }    
    ...
    Maybe<ASTNodeUP> Parser::ParseBinOpExpr() {
        ...
        return{ ParseStatus::Continue, std::make_unique<BinaryOp>(lhs.value()->Tok(),
                                                                  std::move(lhs.value()),
                                                                  std::move(rhs.value()),
                                                                  op.value()) };
    }
    ...
    Maybe<ASTNodeUP> Parser::ParseFunc() {
        ...
        while (true) {
            ...
            params.push_back(std::make_unique<VarDecl>(type.value().Tok(), type.value(), Id{id.value()}));
